...
- 配置页本地存储：新增 IndexedDB 内容体积、站点总占用、浏览器配额和各对象仓库明细，需验证首次打开、刷新统计和中英文显示正常。
- 本地图片清理：执行未使用图片清理时会同时保留生图与视频生成历史引用的图片，需验证清理后历史记录预览仍可打开。
- 提示词来源：新增 Freestylefly GPT Image 2 内置来源，需验证提示词库可以拉取、缓存并展示该来源内容。
- Canvas Agent Codex 兼容：内置 `@openai/codex` 从 `0.139.x` 升级到 `0.144.5`，需验证网页 Agent 面板使用 `gpt-5.6-sol` 发起对话时不再返回“requires a newer version of Codex”。
- 上游安全增量更新：画布空白区域双击可打开文本、图片、视频、音频和配置节点创建菜单；菜单与弹层内滚动不应缩放画布；非 HTTPS 局域网打开时本地 Agent 不应因 `crypto.randomUUID` 不可用而异常；Gemini 生图比例匹配不应因内置的 1:4、1:8 比例误报。
- Duomi Adapter：新增服务端异步文生图、Cloudflare R2/S3 参考图编辑、VEO/Grok/可灵多图与 Omni 视频适配、单图预上传与 JSON URL 模式、统一 JSON 错误、raw/bearer 鉴权、同域 Nginx/Vite 代理和三服务 Compose；28 项 Adapter 测试、12 项 Cloudflare Miniflare/R2 测试、三端类型检查与构建、本地双进程代理和 Wrangler SPA/API 冒烟测试均已通过，仍需使用真实 Duomi/R2 凭据生成可灵视频，并在安装 Docker 的环境验证 Compose。
