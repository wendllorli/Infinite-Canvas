...
- Duomi Adapter：新增服务端异步文生图、Cloudflare R2/S3 参考图编辑、VEO/Grok/可灵视频适配、单图预上传与 JSON URL 模式、统一 JSON 错误、raw/bearer 鉴权、同域 Nginx/Vite 代理和三服务 Compose；26 项 Adapter 测试、11 项 Cloudflare Miniflare/R2 测试、三端类型检查与构建、本地双进程代理和 Wrangler SPA/API 冒烟测试均已通过，仍需使用真实 Duomi/R2 凭据生成可灵视频，并在安装 Docker 的环境验证 Compose。
