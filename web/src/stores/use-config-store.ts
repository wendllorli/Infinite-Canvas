    ...
    persist(
        ...
        }),
        {
            ...
            merge: (persisted, current) => {
                ...
                const config = migrateLegacyImageModel({ ...defaultConfig, ...persistedConfig });
...
export function normalizeChannelModels(models: Array<string | ChannelModel> | undefined): ChannelModel[] {
    ...
    for (const item of models || []) {
        ...
        const name = rawName === LEGACY_IMAGE_MODEL ? DEFAULT_IMAGE_MODEL : rawName;
...
function migrateLegacyImageModel(config: AiConfig): AiConfig {
    const replace = (value: string) => value.replace(new RegExp(`(^|${CHANNEL_MODEL_SEPARATOR})${LEGACY_IMAGE_MODEL}$`), `$1${DEFAULT_IMAGE_MODEL}`);
    return {
        ...config,
        model: replace(config.model),
        imageModel: replace(config.imageModel),
        models: config.models.map(replace),
        channels: config.channels.map((channel) => ({ ...channel, models: normalizeChannelModels(channel.models) })),
    };
}
