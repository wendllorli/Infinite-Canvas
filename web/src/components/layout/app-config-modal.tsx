...
import { createChatgptDiscountChannel, createModelChannel, modelOptionsFromChannels, normalizeModelOptionValue, selectableModelsByCapability, useConfigStore, type AiConfig, type ApiCallFormat, type ConfigTabKey, type ModelCapability, type ModelChannel } from "@/stores/use-config-store";
...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const addChatgptDiscountChannel = () => {
        const existing = config.channels.find((channel) => channel.name === "chatgpt-特价版");
        if (existing) {
            setEditingChannelId(existing.id);
            return;
        }
        const channel = createChatgptDiscountChannel();
        updateChannels([...config.channels, channel]);
        setEditingChannelId(channel.id);
    };
                        ...
                        children: (
                            <div>
                                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                                    ...
                                    <div className="flex gap-2">
                                        <Button onClick={addChatgptDiscountChannel}>chatgpt-特价版</Button>
