...
const ARK_IMAGE_MODEL_ALIASES: Record<string, string> = {
    "doubao-seedream-5-0lite": "doubao-seedream-5-0-260128",
    "doubao-seedream-5-0pro": "doubao-seedream-5-0-pro-260628",
};
...
function arkImagePayload(config: AiConfig, prompt: string, images?: string[]) {
    ...
    if (requestConfig.apiFormat === "ark") {
        try {
            const response = await axios.post<ImageApiResponse>(
                ...
                arkImagePayload({ ...config, ...requestConfig }, prompt),
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (requestConfig.apiFormat === "ark") {
        ...
        try {
            const response = await axios.post<ImageApiResponse>(
                aiApiUrl(requestConfig, "/images/generations"),
                arkImagePayload({ ...config, ...requestConfig }, requestPrompt, refs),
                {
                    headers: aiHeaders(requestConfig, "application/json"),
                    signal: options?.signal,
                },
    ...
    if (isDuomiAdapterBaseUrl(requestConfig.baseUrl)) {
        try {
            ...
            const image = await uploadDuomiImages(requestConfig, files, options?.signal);
            const response = await axios.post<ImageApiResponse>(
            ...
            return parseImagePayload(response.data).map((item) => ({ ...item, dataUrl: duomiResultImageUrl(requestConfig.baseUrl, item.dataUrl) }));
        } catch (error) {
            throw new Error(readAxiosError(error, apiText("requestFailed")));
        }
    }
