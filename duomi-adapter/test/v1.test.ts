...
describe("V1 routes", () => {
    it("returns health and configured model", async () => {
        ...
        expect((await app.inject({ method: "GET", url: "/v1/models" })).json().data.map((item: { id: string }) => item.id)).toEqual(["gpt-image-2.5-flare", "veo3.1-fast", "veo3.1-pro", "grok-video", "grok-video-1.5", "kling-v1-6", "kling-v3-omni"]);
        await app.close();
    });
