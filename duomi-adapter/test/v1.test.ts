...
async function injectGeneration(apiBase: string, overrides: Partial<AdapterConfig> = {}, payload: Record<string, unknown> = {}) {
    ...
    const response = await app.inject({
        ...
        payload: { model: "gpt-image-2.5-flare", prompt: "a lighthouse", size: "16:9", quality: "high", n: 3, response_format: "b64_json", output_format: "png", ...payload },
    });
    await app.close();
    return response;
}
...
describe("V1 routes", () => {
    it("returns health and configured model", async () => {
        ...
        expect((await app.inject({ method: "GET", url: "/v1/models" })).json().data.map((item: { id: string }) => item.id)).toEqual(["gpt-image-2.5-flare", "gpt-image-2.5-sunburst", "veo3.1-fast", "veo3.1-pro", "grok-video", "grok-video-1.5", "kling-v1-6", "kling-v3-omni"]);
        await app.close();
    });
    ...
    it("creates a task, polls pending and running, then returns every image", async () => {
        ...
        const base = await mockServer((request, response, body) => {
            if (request.method === "POST") {
                ...
                expect(JSON.parse(body)).toEqual({ model: "gpt-image-2.5-flare", prompt: "a lighthouse", size: "16:9", quality: "high" });
