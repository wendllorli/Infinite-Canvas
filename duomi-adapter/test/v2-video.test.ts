...
describe("V2 image edits", () => {
    ...
    it.each([1, 3])("forwards %i pre-uploaded public image URL(s)", async (count) => {
        ...
        const response = await fetch(`${base}/v1/images/edits`, {
            ...
            body: JSON.stringify({ model: "gpt-image-2.5-flare", prompt: "change the sky", size: "16:9", quality: "high", image: urls }),
        });
        expect(response.status).toBe(200);
        expect(submitted).toEqual({ model: "gpt-image-2.5-flare", prompt: "change the sky", size: "16:9", quality: "high", image: urls });
        expect(storage.files).toHaveLength(0);
    });
});
