...
`DUOMI_API_BASE`、鉴权模式、模型和轮询参数是 `wrangler.jsonc` 中的非敏感变量。免费版默认轮询间隔为 25000ms，十分钟任务最多使用 41 次 Duomi 上游请求。`/v1/media` 会通过同域代理读取 Duomi 结果图片，避免部分 OSS 临时地址缺少 CORS 响应头而无法写入画布。
...
豆包 Seedream Lite、Pro 与 Seedance 2.0 Pro、2.0 Fast、2.0 Mini、2.5 共用 `ARK_API_KEY`，前端采用原生 v0.19.0，需要手动把渠道地址配置为 `/api/ark/api/v3`，或通过模型自定义调用脚本接入，真实密钥不会进入浏览器或 GitHub。Seedance 只开放 16:9、9:16 和 720p；标准 2.0 额外开放 1080p。
