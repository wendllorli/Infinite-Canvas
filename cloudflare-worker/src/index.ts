...
export async function handleRequest(request: Request, env: Env, fetchImpl: typeof fetch = fetch): Promise<Response> {
    ...
    if (request.method === "GET" && path === "/v1/models") {
        return json({ object: "list", data: [config.imageModel, "gpt-image-2.5-sunburst", ...config.videoModels].map((id) => ({ id, object: "model", owned_by: "duomi" })) });
    }
...
function workerConfig(env: Env): AdapterConfig {
    ...
    return {
        ...
        imageModel: env.DUOMI_IMAGE_MODEL?.trim() || "gpt-image-2.5-flare",
